...
from .utils import is_community_admin, is_system_admin
...
class CommunityViewSet(viewsets.ModelViewSet):
    ...
    def set_role(self, request, slug=None):
        ...
        if not (is_system_admin(request.user) or is_community_admin(request.user, community)):



            raise PermissionDenied("你没有权限管理该社区成员")



        if role == CommunityMembership.ROLE_ADMIN and not is_system_admin(request.user):



            raise PermissionDenied("只有系统管理员可以任命社区管理员")
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not community:
            raise serializers.ValidationError({"community": "请先选择一个社区"})
